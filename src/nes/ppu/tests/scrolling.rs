...
fn test_vertical_reload_preserves_horizontal_bits() {
    ...
    // Set horizontal bits in v that should be preserved
    ppu.v_address.set_coarse_x(20);
    ppu.v_address.0 |= 0x0400; // Set nametable X bit

    // Set vertical bits in t
    ppu.t_address.set_coarse_y(15);
    ppu.t_address.set_fine_y(3);

    // Advance through pre-render vertical reload
    advance_ppu_to(&mut ppu, PRE_RENDER_SCANLINE, 304);

    // Horizontal bits should be preserved
    assert_eq!(
        ppu.v_address.coarse_x(),
        20,
        "Coarse X should be preserved during vertical reload"
    );
}
...
fn test_full_scanline_scroll_sequence() {
    ...
    let initial_coarse_y = ppu.v_address.coarse_y();
    let initial_fine_y = ppu.v_address.fine_y();

    // Advance through one complete visible scanline
    advance_ppu_to(&mut ppu, 0, 340);

    // After one scanline:
    // - Coarse X should be back to initial (reloaded at dot 257)
    // - Fine Y should have incremented (at dot 256)

    // Check that coarse_x was reloaded
    assert_eq!(
        ppu.v_address.coarse_x(),
        ppu.t_address.coarse_x(),
        "Coarse X should be reloaded from t at end of scanline"
    );

    // Check that fine_y incremented
