...
mod test {
  ...
  fn test_bpl() {
    ...
    test_instruction!("BPL", Relative, [0x00]{status: 0b00000000} => []{program_counter: 0x0103, clock_counter: 4}, |cpu: &mut CPU<'_>, _opcode: &Opcode| {
      cpu.program_counter = 0x00FD;
      cpu.unclocked_write_u8(0x00FD, 0x10);
      cpu.unclocked_write_u16(0x00FE, 5);
    });
    test_instruction!("BPL", Relative, [0x00]{status: 0b00000000} => []{program_counter: 0x0100, clock_counter: 4}, |cpu: &mut CPU<'_>, _opcode: &Opcode| {
      cpu.program_counter = 0x00FD;
      cpu.unclocked_write_u8(0x00FD, 0x10);
